...
void GPIO_ExtraExternalButtonAndLedsInit (void){
    ...
    GPIO_PORTB_AMSEL_REG &= (0b11110000);                             /* Disable Analog on PB0123 */
    GPIO_PORTB_PCTL_REG  &= 0xFFFF0000;                       /* Clear PMCx bits for PB0 to use it as GPIO pins */
    GPIO_PORTB_DIR_REG   &= ~(1<<0) ;                /* Configure PB0  as input pins */
    GPIO_PORTB_DIR_REG   |=((1<<1) | (1<<2) | (1<<3) ) ;//For Leds
    GPIO_PORTB_AFSEL_REG &= (0b11110000);                             /* Disable alternative function on PB0123 */
    GPIO_PORTB_PUR_REG   |= ((1<<0));                  /* Enable pull-up on PB0  */
    GPIO_PORTB_DEN_REG   |= 0b00000111;                             /* Enable Digital I/O on PB0 */
}
void ExternalGPIO_RedLedOn(void)
{
    GPIO_PORTB_DATA_REG |= (1<<1);  /* Red LED ON */
}
void ExternalGPIO_BlueLedOn(void)
{
    GPIO_PORTB_DATA_REG |= (1<<2);  /* Blue LED ON */
}

void ExternalGPIO_GreenLedOn(void)
{
    GPIO_PORTB_DATA_REG |= (1<<3);  /* Green LED ON */
}
...
{
    GPIO_PORTB_DATA_REG &= ~(1<<1);  /* Red LED OFF */
}
void ExternalGPIO_BlueLedOff(void)
{
    GPIO_PORTB_DATA_REG &= ~(1<<2);  /* Blue LED OFF */
}
void ExternalGPIO_GreenLedOff(void)
{
    GPIO_PORTB_DATA_REG &= ~(1<<3);  /* Green LED OFF */
}
